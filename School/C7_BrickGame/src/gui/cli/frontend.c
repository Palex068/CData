...
void refreshScreen(WINDOW* win, WINDOW* next) {
  drawInfo(updateCurrentState());  ///< Выводим информацию о состоянии игрового
                                   ///< процесса
  drawField(win);  ///< Выводим "стакан"
  drawNext(next);  ///< Выводим окно следующей фигуры
}
 ...
 * (`KEY_LEFT`).
 * @note noecho(); Процедура управляет тем, отображаются ли на экране символы,
 * вводимые по `getch()`
...
/**
 * @brief Инициализация списка рекордов и запись рекорда при завершении
 * программы.
 */
void initScore() {
  loadScore();
...
void destroyWindow(WINDOW* win) {
  /**
   * @brief `delwin(win)` Удаляет указанное окно, освободив всю память,
   * связанную с ним. В случае перекрытия окон сначала должны быть удалены окна,
   * созданные с помощью процедур `subwin` или `subpad`
   */
  delwin(win);

  /**
   * @brief Процедура `clear()` помещают пробелы во все позиции окна и
   * дополнительно вызывают процедуру `clearok` `clear()` является макросом.
   */
  clear();

  /**
   * @brief Процедура refresh() (или процедуры `prefresh`, `pnoutrefresh`,
   * `wnoutrefresh` или `doupdate`) должны вызываться для реального вывода на
   * экран, тогда как большинство остальных процедур только манипулируют
   * структурами данных. Процедура `wrefresh` копирует указанное окно на экран
   * терминала, принимая во внимание то, что на экран уже выведено, чтобы не
   * выводить информацию повторно (это называется оптимизацией вывода).
   * Процедура `refresh` делает то же самое для стандартного окна `stdscr`. Если
   * не установлен режим `leaveok`, физический курсор терминала помещается на
   * текущее место в окне. Процедура возвращает количество символов, выведенных
   * на терминал. `refresh` является макросом.
   */
  refresh();
}
...
void drawField(WINDOW* win) {
   ...
   * @note По краю окна `win` рисуется рамка. Аргументы `vertch` и `horch` - это
   * символы, с помощью которых рисуются вертикальные и горизонтальные линии.
   * Если `vertch` и `horch` равны `0`, то используются символы по умолчанию -
   * `ACS_VLINE` и `ACS_HLINE` соответственно.
   */
  box(win, 0, 0);

  /**
   * @brief В дополнение к рамке из `<curses.h>` размещаем на экране символы из
   * оригинального Тетриса
   *
   */

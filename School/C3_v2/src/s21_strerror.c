 ...
 * and returns a pointer to an error message string. You 
 * need to declare macros containing arrays of error messages 
 * for mac and linux operating systems. Error descriptions 
 * are available in the original library. Checking the 
 * current OS is carried out using directives.
 * 
 * char *strerror(int errnum)	
 * 
 * Выполняет поиск во внутреннем массиве номера ошибки 
 * errnum и возвращает указатель на строку с сообщением 
 * об ошибке. Нужно объявить макросы, содержащие массивы 

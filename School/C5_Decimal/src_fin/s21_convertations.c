...
// ошибка конвертации
/** @brief Преобразование числа decimal в число типа int
 * @param src исходное число
 * @param dst указатель на число типа decimal
 * @return код ошибки: 0 — OK, 1 — ошибка конвертации.
 */
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  ...
  while (get_exp(src) > 0 && (result = is_zero(src)) == 0) {
    ...
    division_big_decimal_with_rest_10(big_src, &big_src);
...
/** @brief Проверка старших бит числа типа `decimal`
 * @param val int - исходное число типа `decimal`
 * @return если не ноль то возвращает 1
 */
...
/** @brief Преобразование числа float в число типа decimal
 * @param src int - исходное число
 * @param dst *s21_decimal - указатель на число типа decimal
 * @return код ошибки: 0 — OK, 1 — ошибка конвертации.
 */
...
/** @brief Получаем экспоненту числа float
 * @param src float - исходное число
 * @return int - экспонента числа float
 */
...
/** @brief Считаем коэффициент масштабирования (scale) для float
 * @param src float - исходное число
 * @return int - scale числа float
 */
...
/** @brief Считаем количество цифр в целой части для float
 * @param src float - исходное число
 * @return int - количество цифр в целой части числа float
 */
...
/** @brief Изменяет масштаб числа (добавляет либо удаляет указанное число
 * значащих цифр) для double
 * @param copy_num double - исходное число
 * @param scale int коэффициент масштабирования (scale) для float
 * @param count_significant_digits int - количество цифр в целой части числа
 * float
 */
...
/** @brief Запись числа типа `double` в число типа `decimal`
 * @param src Число типа `double`
 * @param dst Число типа `decimal`
 * @param scale Масштаб числа `int`
 * @return код ошибки: 0 — OK, 1 — ошибка конвертации.
 */
int write_float_to_decimal(double src, s21_decimal *dst, int scale) {
  ...
  while (src >= 1 && bit_position < BITS_IN_3_INT) {
    set_bit_int(dst, bit_position, fmodf(src, 2.0));
    src /= 2.0;
    bit_position++;
  }
...
//
/** @brief Убираем не значащие нули
 * @param dst исходное число 'decimal`
 * @param scale - масштаб числа float
 */
void clear_nonsignficant_zero(s21_decimal *dst, int *scale) {
  if (dst != NULL) {
    if (*scale > 0) {
      ...
      do {
        remainder = division_big_decimal_with_rest_10(tmp, &tmp);
        if (remainder == 0) {
          (*scale)--;
          result = tmp;
        }
...
/** @brief Преобразование числа `decimal` в число типа `float`
 * @param src исходное число `decimal`
 * @param dst указатель на число типа `float`
 * @return код ошибки: 0 — OK, 1 — ошибка конвертации.
 */
...
/** @brief Преобразование числа `decimal` в число типа `double`
 * @param src исходное число `decimal`
 * @param dst указатель на число типа `double`
 * @return код ошибки: 0 — OK, 1 — ошибка конвертации.
 */
